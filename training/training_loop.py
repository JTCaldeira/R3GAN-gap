...
def training_loop(
    ...
    if rank == 0:
        ...
        try:
            import wandb
            assert wandb_projname is not None
            assert wandb_groupname is not None
            if rank == 0:
                wandb.init(project=wandb_projname, name=f"{run_dir.split('/')[-1]}", group=wandb_groupname, dir=run_dir)
            log_wandb = True
        except ImportError as err:
    ...
    while True:
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            images = torch.cat([G_ema(z, c).cpu() for z, c in zip(grid_z, grid_c)]).to(torch.float).numpy()
            image_snapshot = save_image_grid(images, os.path.join(run_dir, f'fakes{cur_nimg//1000:09d}.png'), drange=[-1,1], grid_size=grid_size)
